...
export const putCancelBooking = async (appointmentId: string, rut: string) => {
  try {
    ...
    const index = record.appointments.findIndex((appointment) => appointment.id === appointmentId);

    if (index === -1) throw new Error('No se ha encontrado la reserva');

    record.appointments[index].available = true;
    record.appointments[index].patientRut = '';
    record.appointments[index].patientEmail = '';
    record.appointments[index].patientHealthInsurance = '';
    ...
    const { record: record2 }: ApiResponseAppointments = response2.data;
    const patientAppointments: Appointment[] = [];
    const currentDate = new Date();

    for (const appointment of record2.appointments) {
